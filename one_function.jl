...
Pkg.activate(".")
# Pkg.instantiate()
using QuadGK, DifferentialEquations, BenchmarkTools, Trapz
...
constants = (Avog, kappa, beta, elc, epsilon_o, epsilon_w, rho_ion, ionS, boundary, STconst);
...
time_range = range(0.01, boundary, 10001);
conc_matrix = zeros(Float64, size(time_range)[1], 2n_salts)

param = (constants, ion_tot, n_salts, time_range, conc_matrix);
...
function solver(param)
    constants, ion_tot, n_salts, time_range, conc_matrix = param
    Avog, kappa, beta, elc, epsilon_o, epsilon_w, rho_ion, c_bulk, boundary, STconst = constants
    ...
    # u0  = [0.0, 0.0];
    # tspan = (boundary, 0.01);
    bvp = BVProblem(mPBE!, bc!, [0.0, 0.0], (boundary, 0.01));
    sol = solve(bvp, Shooting(RadauIIA5(autodiff=false)))

    function pGM!(time, potential)
        ...
        if n_salts == 1
            ...
            pGM = 4Avog * c_bulk/n_salts*1000 * (gammaconi^2 + gammaconj^2)*1e-20 / (beta*(10e-9)^2)
            ...
            tension = sum(gammacon)/(STconst*n_salts)
            pGM = 4Avog * c_bulk/n_salts*1000 * (gammaconi^2 + gammaconj^2 + gammaconk^2+ gammaconl^2)*1e-20 / (beta*(10e-9)^2)
        end
        return gammacon, tension, pGM
    end;
    sol2 = pGM!(time_range, sol)
    return sol2
end;
