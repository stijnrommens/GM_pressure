...
def size_vel_plot(x, y1, y1_error, y2, y2_error):
    ...
    return plt.show()
size_vel_plot(fiber_probe_results[:,0], fiber_probe_results[:,1], velocity_dispersion, fiber_probe_results[:,4]*1e3, size_dispersion*1e3)
...
def holdup_plot(x, y1, y2):
    ...
    plt.legend()
    return plt.show()
holdup_plot(fiber_probe_results[:,0], pressure_sensor_results[:,-1], fiber_probe_results[:,-1])
